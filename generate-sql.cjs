...
// 3. Move images to public/images/books
const imagesDir = path.join(__dirname, 'public', 'images', 'books');
if (!fs.existsSync(imagesDir)) {
  fs.mkdirSync(imagesDir, { recursive: true });
}

const srcDir = path.join(__dirname, 'Books Details');
const files = fs.readdirSync(srcDir);
for (const file of files) {
  if (file.endsWith('.png') || file.endsWith('.jpg') || file.endsWith('.jpeg')) {
    const srcPath = path.join(srcDir, file);
    const destFile = file.replace(/ /g, '_').replace(/[()]/g, '');
...
let sql = '';
sql += "DELETE FROM books WHERE title NOT LIKE '%sundarkhand%';\n\n";
...
for (const b of books) {
  ...
  sql += `INSERT INTO books (
    ...
    '${escapeString(b.title)}', '${escapeString(b.titleHindi)}', '${escapeString(b.author)}', '${escapeString(b.authorHindi)}', ${b.mrp}, ${b.price}, '${escapeString(b.isbn)}', '${escapeString(b.genre)}', '${escapeString(b.language)}',
    ${b.pages}, '', 5, 0, 1, '${escapeString(b.frontCover)}', '${escapeString(b.backCover)}',
    '', '', '', '${escapeString(b.description)}', '${escapeString(b.descriptionHindi)}',
    0, 0, ${stock}, 0
  );\n`;
}

fs.writeFileSync(path.join(__dirname, 'books_update.sql'), sql, 'utf8');
console.log('Images moved and books_update.sql generated successfully.');
